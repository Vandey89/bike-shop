...
function Carousel({children, width='670px', height='502px'}) {
    ...
    }, [activeSlide]);                                               //budet sledit' za izmeneniem 'activeSlide'

    const nextSlide = () => {
        if(activeSlide < children.length - 1){                      //esli 'activeSlide' budet ne poslednim(sleduushiy)
           setActiveSlide(activeSlide + 1); 
        }
    };

    const prevSlide = () => {
        if(activeSlide > 0 ) {                                        //esli 'activeSlide' budet ne poslednim(predidushiy)
           setActiveSlide(activeSlide - 1); 
        }
    };
